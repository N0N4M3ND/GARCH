...
pacstrap /mnt linux linux-headers linux-firmware base base-devel grub efibootmgr nano vim networkmanager xorg lightdm-webkit2-greeter bspwm sxhkd alacritty ttf-liberation ttf-nerd-fonts-symbols rofi pipewire pipewire-alsa pipewire-pulse pipewire-jack pavucontrol git neofetch htop unzip amd-ucode nvidia && pacstrap /mnt mpv obs-studio firefox blender polybar picom nitrogen &&
