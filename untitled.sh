...
pacstrap /mnt linux linux-headers linux-firmware base base-devel grub efibootmgr nano vim networkmanager openssh xorg lightdm-webkit2-greeter bspwm sxhkd alacritty rofi ttf-liberation ttf-nerd-fonts-symbols pipewire pipewire-alsa pipewire-pulse pipewire-jack pavucontrol git picom nitrogen polybar neofetch htop unzip amd-ucode nvidia && pacstrap /mnt mpv obs-studio firefox blender &&
